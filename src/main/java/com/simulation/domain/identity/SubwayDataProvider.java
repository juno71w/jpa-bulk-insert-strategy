...
public class SubwayDataProvider {
     ...
     */
    public static List<SubwayStatsIdentity> createData(int stations) {
        List<SubwayStatsIdentity> testData = new ArrayList<>();
        ...
        for (int station = 1; station <= stations; station++) {
            for (int min = 0; min < 1440; min += 10) {
                ...
                SubwayStatsIdentity subwayStatsIdentity = new SubwayStatsIdentity("역-" + station, recordTime);
                testData.add(subwayStatsIdentity);
            }
        }
